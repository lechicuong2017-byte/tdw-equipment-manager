...
async function run() {
  ...
  assert.ok(imageFilePicker.includes('tone?: "asset" | "maintenance"'));
  ...
  assert.ok(mediaUpload.includes('tone="asset"'));
  assert.ok(maintenanceForms.includes('tone="maintenance"'));
  assert.ok(maintenanceMediaUpload.includes("Ảnh bảo trì đã lưu"));
  assert.ok(maintenanceMediaUpload.includes('tone="maintenance"'));
  assert.ok(nextMaintenancePage.includes('.eq("owner_type", "MAINTENANCE")'));
  assert.ok(nextMaintenancePage.includes("maintenanceMediaByLog"));
  assert.ok(nextAssetDetailPage.includes('.eq("owner_type", "ASSET")'));
  ...
  assert.ok(appsScript.includes("function verifyBackup(folderId, token)"));
  assert.ok(appsScript.includes("function inspectBackupSpreadsheet_(spreadsheet)"));
  ...
  assert.ok(appsScript.includes("verification.some((item) => !item.matched)"));
  ...
  assert.ok(assetQrLabels.includes("A4 · Dàn nhiều tem"));
  assert.ok(assetQrLabels.includes("Tem 100 × 70 mm · 2 tem mỗi trang"));
  assert.ok(assetQr.includes("/assets/${encodeURIComponent(assetId)}"));
  assert.ok(!assetQrCard.includes("serial_number"));
  ...
  assert.ok(index.includes('tdw-icons.svg?v=20260719#logout'));
  assert.ok(index.includes('tdw-icons.svg?v=20260719#logout-all'));
  assert.ok(index.includes('tdw-icons.svg?v=20260719#overview'));
  assert.ok(index.includes('tdw-icons.svg?v=20260719#software'));
  assert.ok(index.includes('tdw-icons.svg?v=20260719#department'));
  ...
  const verify = await invokeProxy({ fn: "verifyBackup", args: ["backup-folder-id"] }, { cookie: "tdw_session=session-token" });
  assert.equal(verify.res.statusCode, 200);
  assert.deepEqual(JSON.parse(verify.requestToAppsScript.options.body), {
    action: "verifyBackup",
    args: ["backup-folder-id", "session-token"],
    proxy_secret: "proxy-secret",
  });
