...
export default async function SuppliesPage({ searchParams }: SuppliesPageProps) {
  ...
  return <>
    ...
    <section className={`supply-command-bar supply-command-bar--${meta[3]}`}>
      ...
      {section === "catalog" && can(access, "supplies.import") ? <div className="vehicle-actions supply-import-actions"><div className="vehicle-action-group"><small>NHẬP TỪ EXCEL</small><div><ModalTrigger description="Đọc phiếu tổng hợp TDW, tự tạo các hàng hóa chưa có và lưu lịch sử mua sắm." eyebrow="NHẬP DANH MỤC" size="medium" title="Nhập danh mục từ XLSX" triggerClassName="secondary-button" triggerLabel="Nhập danh mục XLSX"><SupplyImportForm /></ModalTrigger><ModalTrigger description="Đọc báo giá nhà cung cấp, tự tạo các hàng hóa chưa có và lưu báo giá để so sánh." eyebrow="BÁO GIÁ" size="large" title="Nhập báo giá XLSX" triggerLabel="Nhập báo giá XLSX"><SupplierQuoteImportForm /></ModalTrigger></div></div></div> : null}
      {section === "quotes" && can(access, "supplies.import") ? <div className="vehicle-actions"><div className="vehicle-action-group vehicle-action-group--primary"><small>NHẬP DỮ LIỆU</small><div><ModalTrigger description="Đọc tự động các mẫu báo giá nhà cung cấp." eyebrow="BÁO GIÁ" size="large" title="Nhập báo giá XLSX" triggerLabel="Nhập báo giá XLSX"><SupplierQuoteImportForm /></ModalTrigger></div></div></div> : null}
    </section>
