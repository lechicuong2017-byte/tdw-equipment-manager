...
export default async function SuppliesPage({ searchParams }: SuppliesPageProps) {
  ...
  return <>
    ...
    {section === "requests" ? <section className="panel supply-panel supply-panel--requests"><div className="panel-heading"><div><p className="eyebrow">PHIẾU YÊU CẦU</p><h2>Mua sắm theo kỳ</h2></div>{can(access, "supplies.import") ? <ModalTrigger description="Đọc phiếu VPP hoặc dụng cụ vệ sinh, kiểm tra trùng và duyệt từng dòng trước khi lưu." eyebrow="NHẬP DỮ LIỆU" size="wide" title="Xem trước lịch sử XLSX" triggerClassName="secondary-button" triggerLabel="Nhập XLSX"><SupplyImportForm /></ModalTrigger> : null}</div><SupplyRequestTable access={access} requests={requests} linesByRequest={linesByRequest} /></section> : null}
