  ...
  return (
    <>
      ...
      {section === "fleet" ? <section className="panel vehicle-section-panel vehicle-section-panel--fleet">
        ...
        <div className="table-wrap"><table><thead><tr><th>Mã / xe</th><th>Biển số</th><th>Tài xế</th><th>Định mức</th><th>Đăng kiểm gần nhất</th><th>Trạng thái</th>{canManage || canDelete ? <th>Thao tác</th> : null}</tr></thead><tbody>
          {vehicles.map((vehicle) => { const inspection = latestInspectionByVehicle.get(vehicle.id); const due = inspection ? dueTone(daysUntil(inspection.expires_on, today)) : null; return <tr key={vehicle.id}><td><strong>{vehicle.vehicle_name}</strong><small>{vehicle.vehicle_code} · {[vehicle.brand, vehicle.model].filter(Boolean).join(" ") || "Chưa có hãng/model"}</small></td><td><strong>{vehicle.license_plate}</strong><small>{vehicle.production_year || "Chưa có năm SX"}</small></td><td>{vehicle.assigned_driver || "—"}<small>{vehicle.departments?.[0]?.name}</small></td><td>{vehicle.fuel_norm_l_per_100km ? `${vehicle.fuel_norm_l_per_100km} l/100 km` : "—"}</td><td>{inspection ? <><strong>{formatDate(inspection.expires_on)}</strong><small><span className={`status-pill ${due?.className}`}>{due?.label}</span></small></> : <span className="text-danger">Chưa có đăng kiểm</span>}</td><td><span className={`status-pill ${vehicle.status === "ACTIVE" ? "status-pill--active" : vehicle.status === "MAINTENANCE" ? "status-pill--attention" : "status-pill--inactive"}`}>{vehicle.status === "ACTIVE" ? "Đang sử dụng" : vehicle.status === "MAINTENANCE" ? "Đang sửa chữa" : vehicle.status === "LIQUIDATED" ? "Đã thanh lý" : "Ngừng sử dụng"}</span></td>{canManage || canDelete ? <td><div className="row-actions">{canManage ? <ModalTrigger description="Cập nhật thông tin nhận diện, phân công và trạng thái xe." eyebrow="HỒ SƠ XE" size="large" title="Sửa hồ sơ xe" triggerClassName="text-button" triggerLabel="Sửa"><VehicleForm departments={departmentsResult.data ?? []} users={usersResult.data ?? []} initial={{ id: vehicle.id, vehicle_code: vehicle.vehicle_code, vehicle_name: vehicle.vehicle_name, license_plate: vehicle.license_plate, brand: vehicle.brand, model: vehicle.model, production_year: vehicle.production_year, fuel_norm_l_per_100km: vehicle.fuel_norm_l_per_100km, assigned_driver: vehicle.assigned_driver, department_id: vehicle.department_id, responsible_user_id: vehicle.responsible_user_id, status: vehicle.status as "ACTIVE" | "MAINTENANCE" | "INACTIVE" | "LIQUIDATED", note: vehicle.note }} /></ModalTrigger> : null}{canDelete ? <ConfirmAction action={deleteVehicleRecord} description="Xe sẽ được ẩn khỏi danh sách quản lý. Toàn bộ đăng kiểm, bảo dưỡng và nhiên liệu đã ghi nhận vẫn được giữ lại." fields={{ id: vehicle.id, kind: "vehicle" }} title="Xóa hồ sơ xe?" /> : null}</div></td> : null}</tr>; })}
          {!vehicles.length ? <tr><td className="empty-cell" colSpan={canManage || canDelete ? 7 : 6}>Chưa có hồ sơ xe. Bạn có thể thêm thủ công hoặc nhập từ file XLSX mẫu.</td></tr> : null}
        </tbody></table></div>
      </section> : null}
      ...
      {section === "inspections" ? <section className="panel vehicle-section-panel vehicle-section-panel--inspections">
        ...
        <div className="table-wrap"><table><thead><tr><th>Xe</th><th>Ngày đăng kiểm</th><th>Ngày hết hạn</th><th>Chi phí</th><th>Thông tin</th>{canManage || canDelete ? <th>Thao tác</th> : null}</tr></thead><tbody>{visibleInspections.map((item) => { const vehicle = relatedVehicle(item.vehicles); const due = dueTone(daysUntil(item.expires_on, today)); return <tr key={item.id}><td><strong>{vehicle?.vehicle_name}</strong><small>{vehicle?.license_plate}</small></td><td>{formatDate(item.inspection_date)}</td><td><strong>{formatDate(item.expires_on)}</strong><small><span className={`status-pill ${due.className}`}>{due.label}</span></small></td><td>{formatMoney(Number(item.cost))}</td><td>{item.certificate_number || "—"}<small>{item.inspection_center || `Nhắc trước ${item.reminder_days} ngày`}</small></td>{canManage || canDelete ? <td><div className="row-actions">{canManage ? <ModalTrigger description="Cập nhật thời hạn, chi phí và thông tin đăng kiểm." eyebrow="ĐĂNG KIỂM" size="large" title="Sửa đăng kiểm" triggerClassName="text-button" triggerLabel="Sửa"><InspectionForm vehicles={vehicleOptions} initial={{ id: item.id, vehicle_id: item.vehicle_id, inspection_date: item.inspection_date, expires_on: item.expires_on, cost: item.cost, reminder_days: item.reminder_days, certificate_number: item.certificate_number, inspection_center: item.inspection_center, odometer_km: item.odometer_km, note: item.note }} /></ModalTrigger> : null}{canDelete ? <ConfirmAction action={deleteVehicleRecord} description="Bản ghi đăng kiểm sẽ bị xóa khỏi lịch sử." fields={{ id: item.id, kind: "inspection" }} title="Xóa đăng kiểm?" /> : null}</div></td> : null}</tr>; })}{!inspections.length ? <tr><td className="empty-cell" colSpan={canManage || canDelete ? 6 : 5}>Chưa có lịch sử đăng kiểm.</td></tr> : null}</tbody></table></div>
