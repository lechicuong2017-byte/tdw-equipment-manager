...
alter table public.export_jobs
  add column if not exists output_format text not null default 'spreadsheet',
...
alter table public.export_jobs
  ...
  check (output_format in ('spreadsheet', 'google_doc', 'pdf'));

alter table public.export_jobs
  drop constraint if exists export_jobs_idempotency_key_check;
alter table public.export_jobs
  add constraint export_jobs_idempotency_key_check
...
create unique index if not exists export_jobs_user_idempotency_idx
  on public.export_jobs (requested_by, idempotency_key)
...
begin
  ...
  if target_output_format not in ('spreadsheet', 'google_doc', 'pdf')
    or coalesce(target_idempotency_key, '') !~ '^[a-f0-9]{64}$'
  then
    raise exception 'Invalid export job request' using errcode = '22023';
  end if;

  if target_output_format in ('google_doc', 'pdf')
    and target_export_type <> 'assets'
  then
    raise exception 'Document reports currently support assets only'
      using errcode = '22023';
  end if;
